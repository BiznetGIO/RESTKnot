...
CREATE TABLE zone (
	...
	zone VARCHAR(100) NULL,
        ...
        user_id SERIAL UNIQUE REFERENCES "user" (id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE record (
	...
	owner VARCHAR(255) NULL,
        ...
        ttl_id SERIAL UNIQUE REFERENCES ttl (id) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE rdata (
        id SERIAL UNIQUE,
        rdata VARCHAR(100) NULL,
        record_id SERIAL UNIQUE REFERENCES record (id) ON DELETE CASCADE ON UPDATE CASCADE
);
